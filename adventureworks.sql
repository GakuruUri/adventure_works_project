...
-- 89. Inventory optimization model (continued from previous)
FROM InventoryOptimization
WHERE MonthsWithData >= 6 -- Ensure sufficient data for reliable calculations
ORDER BY (AvgMonthlySales * StandardCost) DESC; -- Order by inventory value

-- 90. Customer segmentation by RFM (Recency, Frequency, Monetary value)
WITH CustomerPurchases AS (
    SELECT
        c.CustomerID,
        p.FirstName + ' ' + p.LastName AS CustomerName,
        MAX(soh.OrderDate) AS LastPurchaseDate,
        COUNT(DISTINCT soh.SalesOrderID) AS PurchaseFrequency,
        SUM(soh.TotalDue) AS TotalSpend
    FROM Sales.Customer c
    JOIN Person.Person p ON c.PersonID = p.BusinessEntityID
    JOIN Sales.SalesOrderHeader soh ON c.CustomerID = soh.CustomerID
...
SELECT
    ...
    FrequencyScore,
    MonetaryScore,
    -- Combine RFM scores into a single string (e.g., 555, 554, etc.)
    ...
    CAST((RecencyScore * 0.35 + FrequencyScore * 0.35 + MonetaryScore * 0.3) AS DECIMAL(10,2)) AS RFMValue,
    -- Customer segment
    CASE
        ...
        WHEN RecencyScore <= 2 AND FrequencyScore >= 3 AND MonetaryScore >= 3 THEN 'Customers Needing Attention'
        WHEN RecencyScore <= 2 AND FrequencyScore >= 3 AND MonetaryScore <= 2 THEN 'At Risk'
        WHEN RecencyScore <= 2 AND FrequencyScore <= 2 AND MonetaryScore >= 3 THEN 'Hibernating'
        WHEN RecencyScore <= 2 AND FrequencyScore <= 2 AND MonetaryScore <= 2 THEN 'Lost'
        ELSE 'Other'
    END AS CustomerSegment,
    -- Marketing recommendation
    CASE
        WHEN RecencyScore >= 4 AND FrequencyScore >= 4 AND MonetaryScore >= 4 THEN 'Reward program, exclusive offers'
        WHEN RecencyScore >= 4 AND FrequencyScore >= 3 AND MonetaryScore >= 3 THEN 'Loyalty rewards, upsell opportunities'
        WHEN RecencyScore >= 3 AND FrequencyScore >= 1 AND MonetaryScore >= 2 THEN 'Engagement campaign, special discounts'
        WHEN RecencyScore >= 4 AND FrequencyScore <= 2 AND MonetaryScore <= 2 THEN 'Welcome series, education emails'
        WHEN RecencyScore >= 3 AND FrequencyScore <= 2 AND MonetaryScore <= 2 THEN 'Engagement offers, product recommendations'
        WHEN RecencyScore <= 2 AND FrequencyScore >= 3 AND MonetaryScore >= 3 THEN 'Reactivation campaign, special offer'
        WHEN RecencyScore <= 2 AND FrequencyScore >= 3 AND MonetaryScore <= 2 THEN 'Win-back campaign, survey'
        WHEN RecencyScore <= 2 AND FrequencyScore <= 2 AND MonetaryScore >= 3 THEN 'Personalized reactivation offer'
        WHEN RecencyScore <= 2 AND FrequencyScore <= 2 AND MonetaryScore <= 2 THEN 'Last chance offer or exclude from marketing'
        ELSE 'General marketing campaign'
    END AS MarketingRecommendation
FROM RFMScores
ORDER BY RFMValue DESC;

-- 91. Supply chain performance analysis
WITH SupplyChainMetrics AS (
    SELECT
        ...
        -- Quality metrics
        CASE 
            WHEN pod.ReceivedQty = 0 THEN 0 
            ELSE pod.RejectedQty * 100.0 / pod.ReceivedQty 
        END AS DefectRate,
        -- Delivery performance
        CASE 
            WHEN poh.ShipDate <= poh.ShipMethodID -- Using ShipMethodID as a proxy for expected delivery date
            THEN 1 ELSE 0 
...
SELECT
    ...
    COUNT(DISTINCT ProductID) AS UniqueProductsSupplied,
    -- Lead time metrics
    AVG(LeadTime) AS AvgLeadTime,
    STDEV(LeadTime) AS LeadTimeVariability,
    MIN(LeadTime) AS MinLeadTime,
    MAX(LeadTime) AS MaxLeadTime,
    -- Quality metrics
    AVG(DefectRate) AS AvgDefectRate,
    SUM(RejectedQty) AS TotalRejectedUnits,
    SUM(RejectedQty) * 100.0 / NULLIF(SUM(ReceivedQty), 0) AS OverallDefectRate,
    -- Delivery metrics
    SUM(OnTimeDelivery) * 100.0 / COUNT(*) AS OnTimeDeliveryRate,
    -- Cost metrics
    AVG(UnitPrice) AS AvgUnitPrice,
    SUM(LineTotal) AS TotalSpend,
    -- Performance score (weighted average of key metrics)
    (
        (90 - AVG(DefectRate)) * 0.3 + -- 30% weight for quality
        (SUM(OnTimeDelivery) * 100.0 / COUNT(*)) * 0.3 + -- 30% weight for on-time delivery
        (100 - (AVG(LeadTime) / 30 * 100)) * 0.2 + -- 20% weight for lead time (normalized to 100 scale)
        (100 - (STDEV(LeadTime) / AVG(LeadTime) * 100)) * 0.2 -- 20% weight for consistency
    ) AS SupplierPerformanceScore,
    -- Performance category
    CASE
        WHEN (
            (90 - AVG(DefectRate)) * 0.3 +
            (SUM(OnTimeDelivery) * 100.0 / COUNT(*)) * 0.3 +
            (100 - (AVG(LeadTime) / 30 * 100)) * 0.2 +
            (100 - (STDEV(LeadTime) / AVG(LeadTime) * 100)) * 0.2
        ) >= 80 THEN 'Preferred Supplier'
        WHEN (
            (90 - AVG(DefectRate)) * 0.3 +
            (SUM(OnTimeDelivery) * 100.0 / COUNT(*)) * 0.3 +
            (100 - (AVG(LeadTime) / 30 * 100)) * 0.2 +
            (100 - (STDEV(LeadTime) / AVG(LeadTime) * 100)) * 0.2
        ) >= 60 THEN 'Approved Supplier'
        ELSE 'Probationary Supplier'
    END AS SupplierCategory,
    -- Recommendation
    CASE
        WHEN AVG(DefectRate) > 5 THEN 'Quality improvement needed'
        WHEN SUM(OnTimeDelivery) * 100.0 / COUNT(*) < 80 THEN 'Delivery timing improvement needed'
        WHEN AVG(LeadTime) > 20 THEN 'Lead time reduction needed'
        WHEN STDEV(LeadTime) / AVG(LeadTime) > 0.3 THEN 'Consistency improvement needed'
        ELSE 'Maintain relationship'
    END AS PrimaryRecommendation
FROM SupplyChainMetrics
GROUP BY VendorID, VendorName
HAVING COUNT(DISTINCT PurchaseOrderID) >= 5 -- Only analyze vendors with sufficient orders
ORDER BY SupplierPerformanceScore DESC;

-- 92. Product recommendation engine
...
ProductMetrics AS (
    SELECT
        ...
        COUNT(*) * 1.0 / (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader) AS Support,
        -- Confidence (probability of buying product2 given product1)
        COUNT(*) * 1.0 / (SELECT COUNT(*) FROM Sales.SalesOrderDetail WHERE ProductID = ProductID1) AS Confidence,
        -- Lift (ratio of observed support to expected support if products were independent)
        (COUNT(*) * 1.0 / (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader)) /
        ((SELECT COUNT(*) FROM Sales.SalesOrderDetail WHERE ProductID = ProductID1) * 1.0 / 
         (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader) *
         (SELECT COUNT(*) FROM Sales.SalesOrderDetail WHERE ProductID = ProductID2) * 1.0 / 
         (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader)) AS Lift
    FROM ProductPurchases
    GROUP BY ProductID1, ProductName1, ProductID2, ProductName2
)
SELECT
    pm.ProductID1,
    pm.ProductName1,
    pm.ProductID2,
    pm.ProductName2,
    pm.PurchasedTogetherCount,
    pm.Support,
    pm.Confidence,
    pm.Lift,
    -- Rank recommendations for each product
    ROW_NUMBER() OVER (PARTITION BY pm.ProductID1 ORDER BY pm.Lift DESC) AS RecommendationRank,
    -- Recommendation strength
    CASE
        WHEN pm.Lift >= 3 THEN 'Strong Recommendation'
        WHEN pm.Lift >= 2 THEN 'Medium Recommendation'
        ELSE 'Weak Recommendation'
    END AS RecommendationStrength,
    -- Recommendation type
    CASE
        WHEN p1.ProductSubcategoryID = p2.ProductSubcategoryID THEN 'Similar Product'
        WHEN p1.ProductCategoryID = p2.ProductCategoryID THEN 'Complementary Product'
        ELSE 'Cross-Category'
    END AS RecommendationType,
    -- Marketing action
    CASE
        WHEN pm.Lift >= 3 THEN 'Bundle offer with discount'
        WHEN pm.Lift >= 2 THEN 'Show as "Frequently bought together"'
        ELSE 'Show as "You might also like"'
    END AS MarketingAction
FROM ProductMetrics pm
JOIN Production.Product p1 ON pm.ProductID1 = p1.ProductID
JOIN Production.Product p2 ON pm.ProductID2 = p2.ProductID
WHERE pm.PurchasedTogetherCount >= 5 -- Only consider significant associations
ORDER BY pm.ProductID1, RecommendationRank;

-- 93. Customer cohort analysis
WITH CustomerFirstPurchase AS (
    SELECT
        c.CustomerID,
        p.FirstName + ' ' + p.LastName AS CustomerName,
        MIN(soh.OrderDate) AS FirstPurchaseDate,
        FORMAT(MIN(soh.OrderDate), 'yyyy-MM') AS CohortMonth
    FROM Sales.Customer c
    JOIN Person.Person p ON c.PersonID = p.BusinessEntityID
    JOIN Sales.SalesOrderHeader soh ON c.CustomerID = soh.CustomerID
    GROUP BY c.CustomerID, p.FirstName, p.LastName
...
CustomerPurchases AS (
    SELECT
        ...
        soh.OrderDate,
        soh.TotalDue,
        DATEDIFF(MONTH, cfp.FirstPurchaseDate, soh.OrderDate) AS CohortMonth
    FROM CustomerFirstPurchase cfp
    JOIN Sales.SalesOrderHeader soh ON cfp.CustomerID = soh.CustomerID
),
CohortRetention AS (
    SELECT
        CohortMonth,
        ...
        (SELECT COUNT(DISTINCT CustomerID) FROM CustomerFirstPurchase WHERE CohortMonth = cp.CohortMonth) AS CohortSize,
        COUNT(DISTINCT CustomerID) * 100.0 / 
        (SELECT COUNT(DISTINCT CustomerID) FROM CustomerFirstPurchase WHERE CohortMonth = cp.CohortMonth) AS RetentionRate,
        SUM(TotalDue) AS Revenue,
        SUM(TotalDue) / COUNT(DISTINCT CustomerID) AS AverageRevenuePerCustomer
...
SELECT
    ...
    RetentionRate - LAG(RetentionRate) OVER (PARTITION BY CohortMonth ORDER BY MonthNumber) AS RetentionRateChange,
    -- Month-over-month revenue change
    Revenue - LAG(Revenue) OVER (PARTITION BY CohortMonth ORDER BY MonthNumber) AS RevenueChange,
    -- Month-over-month average revenue per customer change
    AverageRevenuePerCustomer - LAG(AverageRevenuePerCustomer) OVER (PARTITION BY CohortMonth ORDER BY MonthNumber) AS ARPCChange,
    -- Retention category
    CASE
        WHEN MonthNumber = 0 THEN 'Initial Purchase'
        WHEN MonthNumber = 1 AND RetentionRate >= 30 THEN 'Strong First Month Retention'
        WHEN MonthNumber = 1 AND RetentionRate < 30 THEN 'Weak First Month Retention'
        WHEN MonthNumber BETWEEN 2 AND 3 AND RetentionRate >= 25 THEN 'Strong Early Retention'
        WHEN MonthNumber BETWEEN 2 AND 3 AND RetentionRate < 25 THEN 'Weak Early Retention'
        WHEN MonthNumber > 3 AND RetentionRate >= 20 THEN 'Strong Long-term Retention'
        WHEN MonthNumber > 3 AND RetentionRate < 20 THEN 'Weak Long-term Retention'
        ELSE 'Other'
    END AS RetentionCategory
FROM CohortRetention
ORDER BY CohortMonth, MonthNumber;

-- 94. Market basket analysis
WITH OrderItems AS (
    SELECT
        soh.SalesOrderID,
        soh.OrderDate,
...
ProductPairs AS (
    SELECT
        ...
        oi1.CategoryName AS Product1Category,
        oi2.ProductID AS Product2ID,
        oi2.ProductName AS Product2Name,
        oi2.CategoryName AS Product2Category
    FROM OrderItems oi1
    JOIN OrderItems oi2 ON 
        oi1.SalesOrderID = oi2.SalesOrderID AND 
        oi1.ProductID < oi2.ProductID -- Ensure unique pairs
),
BasketMetrics AS (
    SELECT
        Product1ID,
        ...
        COUNT(*) AS PairCount,
        -- Support calculation
        COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader) AS SupportPercent,
        -- Confidence calculations
        COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT SalesOrderID) FROM OrderItems WHERE ProductID = Product1ID) AS Confidence1to2,
        COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT SalesOrderID) FROM OrderItems WHERE ProductID = Product2ID) AS Confidence2to1,
        -- Lift calculation
        (COUNT(*) * (SELECT COUNT(DISTINCT SalesOrderID) FROM Sales.SalesOrderHeader)) /
        ((SELECT COUNT(DISTINCT SalesOrderID) FROM OrderItems WHERE ProductID = Product1ID) *
         (SELECT COUNT(DISTINCT SalesOrderID) FROM OrderItems WHERE ProductID = Product2ID)) AS Lift
    FROM ProductPairs
    GROUP BY 
        Product1ID, Product1Name, Product1Category,
        Product2ID, Product2Name, Product2Category
    HAVING COUNT(*) >= 5 -- Only consider significant associations
)
SELECT
    Product1ID,
    Product1Name,
    ...
    CASE
        ...
        WHEN Lift BETWEEN 2 AND 3 THEN 'Strong Association'
        WHEN Lift BETWEEN 1 AND 2 THEN 'Positive Association'
        ELSE 'No Significant Association'
    END AS AssociationStrength,
    -- Marketing recommendation
    CASE
        WHEN Lift > 3 THEN 'Bundle products with discount'
        WHEN Lift BETWEEN 2 AND 3 THEN 'Cross-sell recommendation'
        WHEN Lift BETWEEN 1 AND 2 THEN 'Suggest as related product'
        ELSE 'No specific action'
    END AS MarketingRecommendation,
    -- Category relationship
    CASE
        WHEN Product1Category = Product2Category THEN 'Same Category'
        ELSE 'Cross Category'
    END AS CategoryRelationship
FROM BasketMetrics
ORDER BY Lift DESC;

-- 95. Customer profitability analysis
WITH CustomerOrders AS (
    SELECT
        ...
        soh.OrderDate,
        soh.TotalDue AS Revenue,
        SUM(sod.OrderQty * p.StandardCost) AS ProductCost
    FROM Sales.Customer c
    JOIN Person.Person p ON c.PersonID = p.BusinessEntityID
    JOIN Sales.SalesOrderHeader soh ON c.CustomerID = soh.CustomerID
    JOIN Sales.SalesOrderDetail sod ON soh.SalesOrderID = sod.SalesOrderID
...
CustomerMetrics AS (
    SELECT
        ...
        SUM(Revenue - ProductCost) AS GrossProfit,
        -- Calculate marketing, service, and shipping costs (estimated)
        COUNT(DISTINCT SalesOrderID) * 50 AS MarketingCost, -- $50 per order for marketing
        COUNT(DISTINCT SalesOrderID) * 25 AS ServiceCost, -- $25 per order for customer service
        COUNT(DISTINCT SalesOrderID) * 20 AS ShippingCost, -- $20 per order for shipping
        -- Other overhead (estimated)
        SUM(Revenue) * 0.05 AS OverheadAllocation -- 5% of revenue for general overhead
    FROM CustomerOrders
    GROUP BY CustomerID, CustomerName
)
SELECT
    ...
    TotalCost + MarketingCost + ServiceCost + ShippingCost + OverheadAllocation AS TotalAllCosts,
    -- Calculate net profit
    GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation) AS NetProfit,
    -- Calculate profitability metrics
    (GrossProfit / NULLIF(TotalRevenue, 0)) * 100 AS GrossProfitMargin,
    ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
     NULLIF(TotalRevenue, 0)) * 100 AS NetProfitMargin,
    -- Calculate return on investment
    ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
     NULLIF((MarketingCost + ServiceCost + ShippingCost), 0)) * 100 AS ROI,
    -- Customer profitability segment
    CASE
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 >= 25 THEN 'Highly Profitable'
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 BETWEEN 10 AND 25 THEN 'Profitable'
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 BETWEEN 0 AND 10 THEN 'Marginally Profitable'
        ELSE 'Unprofitable'
    END AS ProfitabilitySegment,
    -- Action recommendation
    CASE
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 >= 25 THEN 'Reward and grow relationship'
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 BETWEEN 10 AND 25 THEN 'Maintain and upsell'
        WHEN ((GrossProfit - (MarketingCost + ServiceCost + ShippingCost + OverheadAllocation)) / 
              NULLIF(TotalRevenue, 0)) * 100 BETWEEN 0 AND 10 THEN 'Analyze for improvement'
        ELSE 'Review relationship'
    END AS ActionRecommendation
FROM CustomerMetrics
ORDER BY NetProfit DESC;

-- 96. Predictive sales forecasting
WITH MonthlySales AS (
    SELECT
        YEAR(soh.OrderDate) AS SalesYear,
        MONTH(soh.OrderDate) AS SalesMonth,
...
SalesMetrics AS (
    SELECT
        ...
        AVG(Revenue) OVER (ORDER BY SalesYear, SalesMonth ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS RevenueMA3,
        AVG(Revenue) OVER (ORDER BY SalesYear, SalesMonth ROWS BETWEEN 5 PRECEDING AND CURRENT ROW) AS RevenueMA6,
        AVG(Revenue) OVER (ORDER BY SalesYear, SalesMonth ROWS BETWEEN 11 PRECEDING AND CURRENT ROW) AS RevenueMA12,
        -- Calculate growth rates
        (Revenue - LAG(Revenue, 1) OVER (ORDER BY SalesYear, SalesMonth)) / 
         NULLIF(LAG(Revenue, 1) OVER (ORDER BY SalesYear, SalesMonth), 0) * 100 AS MonthlyGrowthRate,
        (Revenue - LAG(Revenue, 12) OVER (ORDER BY SalesYear, SalesMonth)) / 
         NULLIF(LAG(Revenue, 12) OVER (ORDER BY SalesYear, SalesMonth), 0) * 100 AS YearlyGrowthRate,
        -- Calculate seasonality factor
        CASE 
            WHEN LAG(Revenue, 12) OVER (ORDER BY SalesYear, SalesMonth) IS NOT NULL
            THEN Revenue / NULLIF(LAG(Revenue, 12) OVER (ORDER BY SalesYear, SalesMonth), 0)
            ELSE 1 
        END AS SeasonalityFactor
    FROM MonthlySales
),
SeasonalityFactors AS (
    SELECT
        SalesMonth,
        AVG(SeasonalityFactor) AS AvgSeasonalityFactor
    FROM SalesMetrics
    WHERE SeasonalityFactor IS NOT NULL AND SeasonalityFactor BETWEEN 0.5 AND 2 -- Filter out extreme values
    GROUP BY SalesMonth
),
GrowthTrend AS (
    SELECT
        AVG(MonthlyGrowthRate) AS AvgMonthlyGrowthRate,
        STDEV(MonthlyGrowthRate) AS StDevMonthlyGrowthRate
    FROM SalesMetrics
    WHERE MonthlyGrowthRate IS NOT NULL AND MonthlyGrowthRate BETWEEN -50 AND 50 -- Filter out extreme values
)
SELECT
    sm.YearMonth,
    sm.Revenue,
    ...
    sm.RevenueMA6,
    sm.RevenueMA12,
    sm.MonthlyGrowthRate,
    sm.YearlyGrowthRate,
    sm.SeasonalityFactor,
    sf.AvgSeasonalityFactor,
    -- Forecast next month
    sm.Revenue * (1 + gt.AvgMonthlyGrowthRate / 100) * 
    LEAD(sf.AvgSeasonalityFactor) OVER (ORDER BY sm.SalesMonth) / sf.AvgSeasonalityFactor AS NextMonthForecast,
    -- Forecast next 3 months
    sm.Revenue * POWER(1 + gt.AvgMonthlyGrowthRate / 100, 3) * 
    LEAD(sf.AvgSeasonalityFactor, 3) OVER (ORDER BY sm.SalesMonth) / sf.AvgSeasonalityFactor AS ThreeMonthForecast,
    -- Forecast next 6 months
    sm.Revenue * POWER(1 + gt.AvgMonthlyGrowthRate / 100, 6) * 
    LEAD(sf.
        ...
        ``` Try if okay ```
        -- 52. Identify products with missing or incomplete data
SELECT 
    p.ProductID,
    p.Name,
    p.ProductNumber,
    ...
    p.Weight,
    CASE
        WHEN p.Color IS NULL THEN 'Missing Color'
        WHEN p.Size IS NULL THEN 'Missing Size'
        WHEN p.Weight IS NULL THEN 'Missing Weight'
        ELSE 'Complete'
    END AS DataStatus
FROM Production.Product p
WHERE p.Color IS NULL OR p.Size IS NULL OR p.Weight IS NULL;

-- 53. Validate email addresses for correct format
SELECT 
    p.BusinessEntityID,
    p.FirstName,
    p.LastName,
    e.EmailAddress,
    CASE
        WHEN e.EmailAddress LIKE '%_@__%.__%' THEN 'Valid'
        ELSE 'Invalid'
    END AS EmailValidity
FROM Person.Person p
JOIN Person.EmailAddress e ON p.BusinessEntityID = e.BusinessEntityID;

-- 54. Check for orphaned records in SalesOrderDetail
...
-- 55. Ensure referential integrity between SalesOrderHeader and Customer
SELECT soh.SalesOrderID, soh.CustomerID
FROM Sales.SalesOrderHeader soh
LEFT JOIN Sales.Customer c ON soh.CustomerID = c.CustomerID
WHERE c.CustomerID IS NULL;

-- 56. Identify customers with invalid phone numbers
SELECT 
    p.BusinessEntityID,
    p.FirstName,
    p.LastName,
    pp.PhoneNumber,
    CASE
        WHEN pp.PhoneNumber LIKE '__________' THEN 'Valid'
        ELSE 'Invalid'
    END AS PhoneValidity
FROM Person.Person p
JOIN Person.PersonPhone pp ON p.BusinessEntityID = pp.BusinessEntityID;

-- 57. Check for products with zero or negative stock levels
SELECT 
    p.ProductID,
    p.Name,
    pi.Quantity,
...
-- 58. Validate addresses for completeness
SELECT 
    a.AddressID,
    a.AddressLine1,
    a.AddressLine2,
    ...
    a.PostalCode,
    CASE
        WHEN a.AddressLine1 IS NULL OR a.City IS NULL OR a.StateProvinceID IS NULL OR a.PostalCode IS NULL THEN 'Incomplete'
        ELSE 'Complete'
    END AS AddressStatus
FROM Person.Address a;

-- 59. Identify employees with missing department information
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    e.DepartmentID,
    CASE
        WHEN e.DepartmentID IS NULL THEN 'Missing Department'
        ELSE 'Complete'
    END AS DepartmentStatus
FROM HumanResources.Employee e
WHERE e.DepartmentID IS NULL;

-- 60. Check for duplicate product names
SELECT 
    p.Name,
    COUNT(*) AS DuplicateCount
FROM Production.Product p
...
-- 61. Validate that all orders have a valid shipping method
SELECT 
    soh.SalesOrderID,
    soh.ShipMethodID,
    CASE
        WHEN sm.ShipMethodID IS NULL THEN 'Invalid Ship Method'
        ELSE 'Valid Ship Method'
    END AS ShipMethodStatus
FROM Sales.SalesOrderHeader soh
LEFT JOIN Purchasing.ShipMethod sm ON soh.ShipMethodID = sm.ShipMethodID
WHERE sm.ShipMethodID IS NULL;

-- 62. Ensure all products have a valid category and subcategory
SELECT 
    p.ProductID,
    p.Name,
    p.ProductSubcategoryID,
    ...
    CASE
        WHEN ps.ProductCategoryID IS NULL THEN 'Invalid Subcategory'
        ELSE 'Valid Subcategory'
    END AS SubcategoryStatus
FROM Production.Product p
LEFT JOIN Production.ProductSubcategory ps ON p.ProductSubcategoryID = ps.ProductSubcategoryID
WHERE ps.ProductCategoryID IS NULL;

-- 63. Identify orders with missing or invalid billing addresses
SELECT 
    soh.SalesOrderID,
    soh.BillToAddressID,
    CASE
        WHEN a.AddressID IS NULL THEN 'Invalid Billing Address'
        ELSE 'Valid Billing Address'
    END AS BillingAddressStatus
FROM Sales.SalesOrderHeader soh
LEFT JOIN Person.Address a ON soh.BillToAddressID = a.AddressID
WHERE a.AddressID IS NULL;

-- 64. Check for products with inconsistent pricing
SELECT 
    p.ProductID,
    p.Name,
    p.ListPrice,
    ph.ListPrice AS HistoricalListPrice,
    CASE
        WHEN p.ListPrice <> ph.ListPrice THEN 'Inconsistent Pricing'
...
-- 65. Validate that all employees have a valid manager
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    e.ManagerID,
    CASE
        WHEN m.BusinessEntityID IS NULL THEN 'Invalid Manager'
        ELSE 'Valid Manager'
...
-- 66. Identify customers with duplicate email addresses
SELECT 
    e.EmailAddress,
    COUNT(*) AS DuplicateCount
...
-- 67. Check for orders with invalid order status
SELECT 
    soh.SalesOrderID,
    soh.Status,
    CASE
        WHEN soh.Status NOT IN (1, 2, 3, 4, 5) THEN 'Invalid Status'
        ELSE 'Valid Status'
    END AS StatusValidity
FROM Sales.SalesOrderHeader soh
WHERE soh.Status NOT IN (1, 2, 3, 4, 5);

-- 68. Ensure all products have a valid supplier
SELECT 
    p.ProductID,
    p.Name,
    pv.BusinessEntityID AS SupplierID,
    CASE
        WHEN v.BusinessEntityID IS NULL THEN 'Invalid Supplier'
        ELSE 'Valid Supplier'
...
-- 69. Identify orders with missing shipping addresses
SELECT 
    soh.SalesOrderID,
    soh.ShipToAddressID,
    CASE
        WHEN a.AddressID IS NULL THEN 'Invalid Shipping Address'
        ELSE 'Valid Shipping Address'
    END AS ShippingAddressStatus
FROM Sales.SalesOrderHeader soh
LEFT JOIN Person.Address a ON soh.ShipToAddressID = a.AddressID
WHERE a.AddressID IS NULL;

-- 70. Check for products with duplicate product numbers
SELECT 
    p.ProductNumber,
    COUNT(*) AS DuplicateCount
FROM Production.Product p
...
-- 71. Validate that all orders have a valid currency rate
SELECT 
    soh.SalesOrderID,
    soh.CurrencyRateID,
    CASE
        WHEN cr.CurrencyRateID IS NULL THEN 'Invalid Currency Rate'
        ELSE 'Valid Currency Rate'
    END AS CurrencyRateStatus
FROM Sales.SalesOrderHeader soh
LEFT JOIN Sales.CurrencyRate cr ON soh.CurrencyRateID = cr.CurrencyRateID
WHERE cr.CurrencyRateID IS NULL;

-- 72. Ensure all employees have a valid hire date
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    e.HireDate,
    CASE
        WHEN e.HireDate IS NULL THEN 'Missing Hire Date'
        ELSE 'Valid Hire Date'
    END AS HireDateStatus
FROM HumanResources.Employee e
WHERE e.HireDate IS NULL;

-- 73. Identify customers with invalid credit card information
SELECT 
    c.CustomerID,
    cc.CardType,
    cc.CardNumber,
    CASE
        WHEN cc.CardNumber LIKE '____________' THEN 'Valid'
        ELSE 'Invalid'
    END AS CardValidity
FROM Sales.Customer c
JOIN Sales.CreditCard cc ON c.CustomerID = cc.CustomerID;

-- 74. Check for orders with invalid discount amounts
SELECT 
    soh.SalesOrderID,
    soh.SubTotal,
    soh.TaxAmt,
    ...
    soh.TotalDue,
    CASE
        WHEN soh.TotalDue < (soh.SubTotal + soh.TaxAmt + soh.Freight) THEN 'Invalid Discount'
        ELSE 'Valid Discount'
    END AS DiscountStatus
FROM Sales.SalesOrderHeader soh
WHERE soh.TotalDue < (soh.SubTotal + soh.TaxAmt + soh.Freight);

-- 75. Ensure all products have a valid safety stock level
SELECT 
    p.ProductID,
    p.Name,
    p.SafetyStockLevel,
    CASE
        WHEN p.SafetyStockLevel IS NULL THEN 'Missing Safety Stock Level'
        ELSE 'Valid Safety Stock Level'
    END AS SafetyStockStatus
FROM Production.Product p
WHERE p.SafetyStockLevel IS NULL;

-- 76. Identify orders with missing or invalid order dates
SELECT 
    soh.SalesOrderID,
    soh.OrderDate,
    CASE
        WHEN soh.OrderDate IS NULL THEN 'Missing Order Date'
        ELSE 'Valid Order Date'
    END AS OrderDateStatus
FROM Sales.SalesOrderHeader soh
WHERE soh.OrderDate IS NULL;

-- 77. Check for products with invalid standard cost
SELECT 
    p.ProductID,
    p.Name,
    p.StandardCost,
    CASE
        WHEN p.StandardCost <= 0 THEN 'Invalid Standard Cost'
        ELSE 'Valid Standard Cost'
    END AS StandardCostStatus
FROM Production.Product p
WHERE p.StandardCost <= 0;

-- 78. Validate that all orders have a valid territory
SELECT 
    soh.SalesOrderID,
    soh.TerritoryID,
    CASE
        WHEN st.TerritoryID IS NULL THEN 'Invalid Territory'
...
-- 79. Ensure all employees have a valid birth date
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    p.BirthDate,
    CASE
        WHEN p.BirthDate IS NULL THEN 'Missing Birth Date'
...
-- 80. Identify customers with invalid addresses
SELECT 
    c.CustomerID,
    a.AddressLine1,
    a.AddressLine2,
    ...
    a.PostalCode,
    CASE
        WHEN a.AddressLine1 IS NULL OR a.City IS NULL OR a.StateProvinceID IS NULL OR a.PostalCode IS NULL THEN 'Invalid Address'
        ELSE 'Valid Address'
    END AS AddressStatus
FROM Sales.Customer c
JOIN Person.Address a ON c.CustomerID = a.AddressID;

-- 81. Check for products with invalid list price
SELECT 
    p.ProductID,
    p.Name,
    p.ListPrice,
    CASE
        WHEN p.ListPrice <= 0 THEN 'Invalid List Price'
        ELSE 'Valid List Price'
    END AS ListPriceStatus
FROM Production.Product p
WHERE p.ListPrice <= 0;

-- 82. Validate that all orders have a valid credit card
SELECT 
    soh.SalesOrderID,
    soh.CreditCardID,
    CASE
        WHEN cc.CreditCardID IS NULL THEN 'Invalid Credit Card'
...
-- 83. Ensure all products have a valid weight
SELECT 
    p.ProductID,
    p.Name,
    p.Weight,
...
-- 84. Identify orders with invalid shipping dates
SELECT 
    soh.SalesOrderID,
    soh.ShipDate,
    CASE
        WHEN soh.ShipDate IS NULL THEN 'Missing Ship Date'
        ELSE 'Valid Ship Date'
    END AS ShipDateStatus
FROM Sales.SalesOrderHeader soh
WHERE soh.ShipDate IS NULL;

-- 85. Check for products with invalid reorder point
SELECT 
    p.ProductID,
    p.Name,
    p.ReorderPoint,
    CASE
        WHEN p.ReorderPoint IS NULL THEN 'Missing Reorder Point'
        ELSE 'Valid Reorder Point'
    END AS ReorderPointStatus
FROM Production.Product p
WHERE p.ReorderPoint IS NULL;

-- 86. Validate that all employees have a valid gender
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    p.Gender,
    CASE
        WHEN p.Gender IS NULL THEN 'Missing Gender'
        ELSE 'Valid Gender'
    END AS GenderStatus
FROM HumanResources.Employee e
JOIN Person.Person p ON e.BusinessEntityID = p.BusinessEntityID
...
-- 87. Identify customers with invalid phone numbers
SELECT 
    c.CustomerID,
    pp.PhoneNumber,
    CASE
    ...
    END AS PhoneValidity
FROM Sales.Customer c
JOIN Person.PersonPhone pp ON c.CustomerID = pp.BusinessEntityID;

-- 88. Ensure all products have a valid product number
SELECT 
    p.ProductID,
    p.Name,
    p.ProductNumber,
    CASE
        WHEN p.ProductNumber IS NULL THEN 'Missing Product Number'
        ELSE 'Valid Product Number'
    END AS ProductNumberStatus
FROM Production.Product p
WHERE p.ProductNumber IS NULL;

-- 89. Check for orders with invalid due dates
SELECT 
    soh.SalesOrderID,
    soh.DueDate,
    CASE
        WHEN soh.DueDate IS NULL THEN 'Missing Due Date'
        ELSE 'Valid Due Date'
    END AS DueDateStatus
FROM Sales.SalesOrderHeader soh
WHERE soh.DueDate IS NULL;

-- 90. Validate that all orders have a valid customer
SELECT 
    soh.SalesOrderID,
    soh.CustomerID,
    CASE
...
-- 91. Ensure all employees have a valid marital status
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    p.MaritalStatus,
    CASE
        WHEN p.MaritalStatus IS NULL THEN 'Missing Marital Status'
        ELSE 'Valid Marital Status'
    END AS MaritalStatusStatus
FROM HumanResources.Employee e
JOIN Person.Person p ON e.BusinessEntityID = p.BusinessEntityID
WHERE p.MaritalStatus IS NULL;

-- 92. Identify customers with invalid email addresses
SELECT 
    c.CustomerID,
    e.EmailAddress,
    CASE
    ...
    END AS EmailValidity
FROM Sales.Customer c
JOIN Person.EmailAddress e ON c.CustomerID = e.BusinessEntityID;

-- 93. Check for products with invalid size
SELECT 
    p.ProductID,
    p.Name,
    p.Size,
...
-- 94. Validate that all orders have a valid sales person
SELECT 
    soh.SalesOrderID,
    soh.SalesPersonID,
    CASE
        WHEN sp.BusinessEntityID IS NULL THEN 'Invalid Sales Person'
        ELSE 'Valid Sales Person'
...
-- 95. Ensure all employees have a valid job title
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    CASE
        WHEN e.JobTitle IS NULL THEN 'Missing Job Title'
        ELSE 'Valid Job Title'
    END AS JobTitleStatus
FROM HumanResources.Employee e
WHERE e.JobTitle IS NULL;

-- 96. Identify customers with invalid addresses
SELECT 
    c.CustomerID,
    a.AddressLine1,
    a.AddressLine2,
...
-- 97. Check for products with invalid color
SELECT 
    p.ProductID,
    p.Name,
    p.Color,
...
-- 98. Validate that all orders have a valid ship method
SELECT 
    soh.SalesOrderID,
    soh.ShipMethodID,
    CASE
...
-- 99. Ensure all employees have a valid department
SELECT 
    e.BusinessEntityID,
    e.JobTitle,
    e.DepartmentID,
...
-- 100. Identify customers with invalid phone numbers
SELECT 
    c.CustomerID,
    pp.PhoneNumber,
    CASE
